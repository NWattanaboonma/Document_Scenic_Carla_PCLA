...
# class SetAIAgentAction(VehicleAction):
#     """
#     Robust AI agent controller with comprehensive dependency handling
#     """
#     def __init__(self, agent_name, route=None):
#         self.agent_name = agent_name
#         self.route = route
#         self._deps_checked = False
#         self._deps_available = False
#         self._pcla_path = Path("C:/Users/bluet/Desktop/Project_Final/PCLA")

#     def _setup_environment(self):
#         """Configure Python path and environment variables"""
#         if str(self._pcla_path) not in sys.path:
#             sys.path.append(str(self._pcla_path))
...
#     def _initialize_pcla(self, obj, sim):
#         """Initialize PCLA controller"""
#         from PCLA import PCLA
        
#         if self.agent_name.startswith("tfpp_l6_"):
#             os.environ["UNCERTAINTY_THRESHOLD"] = "033"
#         elif self.agent_name.startswith("tfpp_lav_"):
#             os.environ["STOP_CONTROL"] = "1"
#         elif self.agent_name.startswith("tfpp_aim_") or self.agent_name.startswith("tfpp_wp_"):
#             os.environ["DIRECT"] = "0"
#         elif self.agent_name == "if_if":
#             os.environ["ROUTES"] = str(self.route)

#         return PCLA(self.agent_name, obj.carlaActor, self.route, sim.client)
...
#         # Fallback to CARLA autopilot if dependencies missing
#         if not self._deps_available:
#             print("Using CARLA autopilot as fallback")
#             obj.carlaActor.set_autopilot(True)
#             return

#         try:
#             # Initialize and control
#             pcla_agent = self._initialize_pcla(obj, sim)
#             control = pcla_agent.get_action()
#             obj.carlaActor.apply_control(control)
            
#         except Exception as e:
#             print(f"PCLA control error: {e}")
#             print("Falling back to basic autopilot")
#             obj.carlaActor.set_autopilot(True)
#--------------------------------------------------------------------------------------------------------------------------------
...
class SetAIAgentAction(VehicleAction):
    ...
    def _initialize_pcla(self, obj, sim):
        ...
        # Pass the selected device to the PCLA controller
        return PCLA(self.agent_name, obj.carlaActor, self.route, sim.client, device=self.device)
